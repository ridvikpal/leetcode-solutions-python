...
class Solution:
    ...
    def isInterleave(self, s1: str, s2: str, s3: str) -> bool:
        ...
        # we will use a dict instead of an array
        # because we need to store tuples (index1, index2)
        memoization = dict()
        ...
        def search(index1, index2) -> bool:
            ...
            # check if we have a memoized value stored
            if (index1, index2) in memoization:
                # if we do, return it immediately
            ...
            # finally we will memoize true if we found a valid
            # path to construct s3 by interleaving s1 and s2
            # from either decision
            memoization[(index1, index2)] = choose_first_char or choose_second_char
            
            # we can return the memoized value we just stored
            return memoization[(index1, index2)]
