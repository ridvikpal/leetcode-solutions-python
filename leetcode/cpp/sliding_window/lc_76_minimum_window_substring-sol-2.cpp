...
public:
    std::string minWindow(std::string s, std::string t) {
        ...
        for (int j = 0; j < s.size(); ++j) {
            ...
            if (tCounter.contains(s[j]) && sSlidingCounter[s[j]] == tCounter[s[j]]) {
                ++validSubstringChars;
            }
            ...
            bool validSubstringFound = validSubstringChars == tCounter.size();
            ...
            while (validSubstringFound) {
                ...
                if (tCounter.contains(s[i]) && sSlidingCounter[s[i]] < tCounter[s[i]]) {
                    --validSubstringChars;
                }
                ...
                validSubstringFound = validSubstringChars == tCounter.size();
            }
        }
