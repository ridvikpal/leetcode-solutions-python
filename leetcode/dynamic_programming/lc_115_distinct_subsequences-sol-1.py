    ...
    def numDistinct(self, s: str, t: str) -> int:
        ...
        def search(index1, index2) -> int:
            ...
            if s[index1] == t[index2]:
                ...
                return search(index1+1, index2) + search(index1+1, index2+1)
            
            # otherwise the chars are not the same, so we
            # can only skip this char in s and stay on
            # the same char in t for our recursive search
            return search(index1+1, index2)
