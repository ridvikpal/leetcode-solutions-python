...
class Solution:
    ...
    def longestIncreasingPath(self, matrix: List[List[int]]) -> int:
        ...
        # init our memoization 2d matrix array
        memoization = [[-1 for _ in range(col_len)] for _ in range(row_len)]
        ...
        # is because the longest increasing path
        ...
        # same paths again
        def dfs(row, col) -> int:
            # first check if already have a memoized value saved
            ...
            # finally at each function call we simply find
            # our max path length for this cell as the sum of
            # 1 (because we visited this cell) + the max
            # path length of the adjacent cells fro this cell
            memoization[row][col] = 1 + max_adjacent_path_length

            # we can return our memoized value we just stored
            return memoization[row][col]
