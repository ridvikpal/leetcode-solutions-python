...
class Solution:
    ...
    def numDecodings(self, s: str) -> int:
        # init our memoization array with all -1s
        memoization = [-1]*len(s)
        ...
        def search(index) -> int:
            ...
            # to optimize, after our bases cases we have to check
            # if we have already found the possible combinations from
            # this index before in our memoization array
            if memoization[index] > -1:
                # if we have, then save time and just return
                # the memoized value
            ...
            if index < len(s)-1 and 10 <= int(s[index:index+2]) <= 26:
                ...
                # and store the result in the memoization array for this index
                memoization[index] = search(index+1) + search(index+2)
            # else we know that the char < 10 so only 1 mapping exists
            else:
                # go normally to the next index recrusively
                # and store the result in the memoization array
                memoization[index] = search(index+1)

            # finally we will return the value in the memoization index
            # because we will have updated it
            return memoization[index]
