...
class Solution:
    ...
    def networkDelayTime(self, times: List[List[int]], n: int, k: int) -> int:
        ...
        for source, target, time in times:
            ...
            # note we subtract 1 from source and target
            # because our min_times array later on is 0-indexed
            adjacency_list[source-1].append((target-1, time))

        # init our min_times array
        # that will hold the minimum time to reach each
        # node using Dijkstra's algorithm.
        min_times = [math.inf]*n

        # init our minimum time for our starting node k to 0
        # we subtract 1 from k because the array is 0-indexed
        min_times[k-1] = 0
